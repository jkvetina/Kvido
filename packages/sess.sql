CREATE OR REPLACE PACKAGE BODY sess AS
    ...
    BEGIN
        -- update timer stored at page start to track page performance
        IF COALESCE(in_log_id, apex_log_id) IS NOT NULL THEN
            tree.update_timer(COALESCE(in_log_id, apex_log_id));
        END IF;
        --
        DBMS_SESSION.CLEAR_IDENTIFIER();
